...
handle_call({register, Pid}, _From, {MonitorRefs, Pids}) ->
    %% io:format("~p:~p - ~p,~p~n", [?MODULE, 'handle_call - register', MonitorRefs, Pids]),
...
handle_cast({increment, Pid}, {MonitorRefs, Pids}) ->
    io:format("~p:~p - ~p,~p,~p~n", [?MODULE, "connected", Pid, MonitorRefs, Pids]),
    {noreply, {MonitorRefs, [Pid | Pids]}};

handle_cast({decrement, Pid}, {MonitorRefs, Pids}) ->
    case catch lists:delete(Pid, Pids) of
        {'EXIT, _'} ->
            {noreply, {MonitorRefs, Pids}};
        NewPids ->
            catch io:format("~p:~p - ~p,~p,~p~n",
                            [?MODULE, "disconnected", Pid, MonitorRefs, Pids]),
            {noreply, {MonitorRefs, NewPids}}
    end;
